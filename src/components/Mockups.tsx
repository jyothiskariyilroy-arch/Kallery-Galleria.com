...
function MockupImage({ src, alt }: { src: string; alt: string }) {
  ...
  if (error) {
    return (
      <div className="img-placeholder w-full h-full flex flex-col items-center justify-center gap-3 min-h-[260px]">
        <div className="text-gold/20 text-4xl">◈</div>
        <p className="label opacity-20 text-center">AI Mockup</p>
      </div>
    );
  }
  return (
...
export default function Mockups() {
  ...
  return (
    <section ref={ref} id="mockups" className="relative py-40 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="mb-16 flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          ...
          <motion.p className="text-ivory/25 font-dm text-sm max-w-xs leading-relaxed"
            ...
            AI-rendered visualisations of how each floor could be configured for different business types.
          </motion.p>
        </div>
        ...
        <motion.div
          ...
          {images.mockups.map((src, i) => (
            <motion.div
              ...
              {/* Overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-obsidian-950/80 via-obsidian-950/20 to-transparent pointer-events-none" />

              {/* Label */}
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <p className="font-cormorant text-xl text-ivory/90 leading-tight">
                  {mockupLabels[i]?.title}
                </p>
                <p className="font-mono text-[0.55rem] uppercase tracking-widest text-gold/60 mt-1">
                  {mockupLabels[i]?.sub}
                </p>
              </div>
              ...
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span className="font-mono text-[0.55rem] uppercase tracking-widest text-gold/70 bg-obsidian-950/70 px-2 py-1 rounded">
                  AI Rendered
                </span>
              </div>
            </motion.div>
        ...
        <motion.div className="mt-14 text-center"
          ...
          <p className="text-ivory/25 font-dm text-sm mb-6">
            Every floor built for commercial intensity — adaptable to your vision.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="tel:+919495040824" className="btn-gold">Schedule Site Visit</a>
            <a href="https://wa.me/919495040824?text=Hi%2C%20I%27m%20interested%20in%20Kallery%20Galleria.%20Please%20share%20the%20mockup%20deck."
