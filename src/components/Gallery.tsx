...
// Grid positions drive which sizes hint to use
const gridItems = [
  { colSpan: "col-span-7", rowSpan: "row-span-2", sizes: "(max-width: 768px) 100vw, (max-width: 1200px) 58vw, 800px" },
  { colSpan: "col-span-5", rowSpan: "row-span-1", sizes: "(max-width: 768px) 100vw, (max-width: 1200px) 42vw, 560px" },
  { colSpan: "col-span-5", rowSpan: "row-span-1", sizes: "(max-width: 768px) 100vw, (max-width: 1200px) 42vw, 560px" },
  { colSpan: "col-span-4", rowSpan: "row-span-1", sizes: "(max-width: 768px) 50vw, (max-width: 1200px) 33vw, 420px" },
  { colSpan: "col-span-4", rowSpan: "row-span-1", sizes: "(max-width: 768px) 50vw, (max-width: 1200px) 33vw, 420px" },
  { colSpan: "col-span-4", rowSpan: "row-span-1", sizes: "(max-width: 768px) 50vw, (max-width: 1200px) 33vw, 420px" },
];
...
const alts = [
  "Kallery Galleria exterior — main road frontage",
  "Ground floor interior — 4,000 sq ft open space",
  "1st floor — open plan layout",
  "2nd floor — professional finish",
  "3rd floor — elevated view",
  "Interior detail — premium finishes",
];
    ...
    <section ref={ref} id="gallery" className="relative py-32 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-12">
        <div className="mb-14 flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          ...
          <motion.p
            className="text-ivory/25 font-dm text-sm max-w-xs leading-relaxed"
            initial={{ opacity: 0 }}
            animate={inView ? { opacity: 1 } : {}}
        ...
        >
          {imgs.slice(0, 6).map((src, i) => (
            <GalleryImage
              ...
              src={src}
              alt={alts[i]}
              sizes={gridItems[i].sizes}
              className={`${gridItems[i].colSpan} ${gridItems[i].rowSpan}`}
            />
          ))}
        </motion.div>
        ...
        >
          {captions.map((cap) => (
            <p
              key={cap}
              className="font-mono text-[0.55rem] uppercase tracking-widest text-ivory/20 text-center"
            >
              {cap}
            </p>
          ))}
